...
export function AuthModal({
  ...
  const handleEmailAuthSubmit = async (e: FormEvent) => {
    ...
    try {
      if (!authMail || !authPassword) throw new Error('Please enter email and password.');
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0, y: 15 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 15 }} className="relative w-full max-w-md overflow-hidden rounded-lg border border-cyan-500/30 bg-[#0a0c10] text-cyan-400 shadow-[0_0_40px_rgba(6,182,212,0.15)]" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-cyan-950 bg-cyan-950/10 p-5">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-xs font-mono font-bold uppercase tracking-[0.2em] text-white">{isSignUpMode ? '云端授权账号注册' : '云端同步账号登录'}</h3>
                  <p className="mt-0.5 text-[8px] font-mono uppercase tracking-wider text-cyan-600">SUPPORT QQ, 163, GMAIL & SYSTEM CLOUD SYNC</p>
                </div>
              </div>
            ...
            <div className="space-y-4 p-6">
              <div className="space-y-2">
                <span className="mb-1 block text-[8px] font-mono uppercase tracking-widest text-cyan-600">OPTION 1 : ONE-CLICK GOOGLE SECURE LINK</span>
                <button type="button" disabled={isGoogleSubmitting} onClick={handleGoogleAuthSubmit} className="flex w-full items-center justify-center gap-2.5 rounded border border-cyan-900/60 bg-cyan-950/20 p-2.5 text-[10px] font-mono uppercase tracking-widest text-cyan-300 shadow-[inset_0_0_8px_rgba(6,182,212,0.05)] transition-all hover:border-cyan-400/50 hover:bg-cyan-900/30">
                  {isGoogleSubmitting ? '正在连接 Google...' : 'Google 谷歌账号一键连接'}
                </button>
              </div>

              <div className="relative flex items-center py-1">
                <div className="flex-grow border-t border-cyan-950" />
                <span className="mx-3 flex-shrink text-[8px] font-mono uppercase tracking-widest text-cyan-700/80">或者</span>
                <div className="flex-grow border-t border-cyan-950" />
              </div>

              <form onSubmit={handleEmailAuthSubmit} className="space-y-3">
                <span className="mb-1 block text-[8px] font-mono uppercase tracking-widest text-cyan-600">OPTION 2 : EMAIL SECURE ID CREDENTIAL</span>
                {isSignUpMode && (
                  <div className="space-y-1">
                    <label className="block text-[9px] font-mono uppercase tracking-wider text-cyan-500">用户昵称 / NICKNAME (选填)</label>
                    <input value={authDisplayName} onChange={(event) => setAuthDisplayName(event.target.value)} placeholder="给自己起一个极客代号..." className="w-full rounded border border-cyan-950 bg-[#050608] px-3 py-2 text-xs font-sans text-cyan-300 outline-none transition-colors placeholder:text-cyan-800 hover:border-cyan-800 focus:border-cyan-500" />
                  </div>
                )}
                <div className="space-y-1">
                  <label className="block text-[9px] font-mono uppercase tracking-wider text-cyan-500">邮箱地址 / EMAIL ADDRESS</label>
                  <div className="relative">
                    <Mail className="absolute left-3 top-2.5 h-3.5 w-3.5 text-cyan-700" />
                    <input type="text" required value={authMail} onChange={(event) => setAuthMail(event.target.value)} placeholder="例如: youraccount@qq.com 或 test@163.com" className="w-full rounded border border-cyan-950 bg-[#050608] py-2 pl-9 pr-3 text-xs font-mono text-cyan-300 outline-none transition-colors placeholder:text-cyan-800 hover:border-cyan-800 focus:border-cyan-500" />
                  </div>
                </div>
                <div className="space-y-1">
                  <label className="block text-[9px] font-mono uppercase tracking-wider text-cyan-500">登录密码 / SECURE PASSWORD</label>
                  <div className="relative">
                ...
                <button type="submit" disabled={isSubmitting} className="flex w-full items-center justify-center gap-2 rounded border border-cyan-500/30 bg-cyan-950/40 p-2.5 text-[10px] font-mono uppercase tracking-[0.2em] text-cyan-300 transition-all hover:border-cyan-400 hover:bg-cyan-500/20 hover:text-white disabled:opacity-50">
                  {isSubmitting ? '处理中...' : isSignUpMode ? 'CREATE SYNC ACCOUNT / 注册云端同步账号' : 'COMMIT CORE LOG-IN / 登录云端同步账号'}
                </button>
              </form>

              <div className="pt-2 text-center text-[9px] font-mono tracking-widest text-cyan-600">
                {isSignUpMode ? (
                  <button type="button" onClick={() => { setIsSignUpMode(false); setLocalError(''); }} className="cursor-pointer underline transition-colors hover:text-cyan-400">[ 已有账号？切换到登录 / RETURN TO SIGN-IN ]</button>
                ) : (
                  <button type="button" onClick={() => { setIsSignUpMode(true); setLocalError(''); }} className="cursor-pointer underline transition-colors hover:text-cyan-400">[ 没有账号？切换到注册 / CREATE A NEW ACCOUNT ]</button>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-2 border-t border-cyan-950/50 bg-cyan-950/10 p-4 text-[8px] font-mono leading-normal text-cyan-700">
              ...
              <div>
                <strong className="mb-0.5 block uppercase text-cyan-600">本地账号系统说明:</strong>
                邮箱注册和登录已切换为本地 PostgreSQL 账号体系，账号数据保存在当前 Docker PostgreSQL 数据卷中。Google 登录走后端 OAuth，需先配置 Google OAuth Client。
              </div>
            </div>
          </motion.div>
        </div>
